...
def create2():
    def show1():
        ...
        if ".txt"in Filepath:
            ...
            top.destroy()
            return 0
        all_the_text = open(Filepath, encoding='UTF-8', errors='ignore').read()
        print(all_the_text)
    ...
    def sayit1():
        ...
        a = engine.say(all_the_text)
        engine.runAndWait()
        engine.stop()

    top = Toplevel()
    top.title('txt提取音频')
    top.geometry("250x70+800+400")
    top.resizable(False,False)
    #richText=tkinter.Text(root,width=380)
    #lbTime=tkinter.Label(top,fg='red',anchor='w')
    #lbTime.place(x=5,y=250,width=150)


    # Button(top, text='出现2级',comand=mp4_mp3()).grid(row=1, column=1, padx=1, pady=1)
    Button(top, text="朗读信息", width=10, command=sayit1) \
        .grid(row=0, column=0, sticky=W, padx=10, pady=5)
    Button(top, text="获取txt文件", width=10, command=show1) \
...
if __name__ == '__main__':
    ...
    Button(root, text='txt提取音频', command=create2).grid(row=0, column=3, sticky=W, padx=10, pady=20)
    Button(root, text="退出",       command=root.quit).grid(row=1, column=4, sticky=E, padx=100, pady=15)
    mainloop()